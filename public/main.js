...
var _user$project$Board$toRectangle = function (board) {
	var _p0 = _elm_lang$core$Native_Utils.cmp(
		...
		_user$project$Board$size) < 0;
	if (_p0 === true) {
		return {ctor: '[]'};
	} else {
		var t = A2(_elm_community$array_extra$Array_Extra$sliceFrom, _user$project$Board$size, board);
		var h = _elm_lang$core$Array$toList(
			A2(_elm_community$array_extra$Array_Extra$sliceUntil, _user$project$Board$size, board));
		return A2(
			_elm_lang$core$Basics_ops['++'],
			{
				ctor: '::',
...
var _user$project$Main$renderPlayer = function (player) {
	...
	return A2(
		...
		{ctor: '[]'},
		{
			ctor: '::',
			_0: _elm_lang$html$Html$text(
				A2(_elm_lang$core$Basics_ops['++'], 'current player: ', playerName)),
			_1: {ctor: '[]'}
		});
};
			...
			function (row, arr) {
				return A2(
					...
					A2(
						...
						F2(
							function (i, c) {
								return A2(renderCell, i + (row * _user$project$Board$size), c);
							}),
						arr));
			});
		var rectangle = _user$project$Board$toRectangle(board);
		return A2(
